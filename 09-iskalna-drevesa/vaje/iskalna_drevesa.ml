...
let rec height drevo = match drevo with
     | Prazno -> 0
     | Vozlisce (l,_,r) -> 1 + max (height l) (height r) 
...
let rec list_of_tree drevo = match drevo with
     | Prazno -> []
     | Vozlisce(l, x, l) -> (list_of_tree l) @ [x] @ (list_of_tree r)
...
(*ni prov*)
let rec is_bst drevo = match drevo with
     | Prazno -> true
     | Vozlisce (l, x, r) -> 
          if l < x and r > x then Vozlisce (is_bst l, x, is_bst r)
          else false
...
let rec delete x = function
     ...
     | Vozlisce(l, x, r) -> 
          if x < y then
               Vozlisce(delete x r, y, r)
          else if x > y then
               Vozlisce(l, y, delete x r)
          else
               let naslednik = succ Vozlisce(l, x, r) in
               match naslednik with
                    | None -> l
                    | Some naslednik -> Vozlisce (, naslednik, delete naslednik r)
